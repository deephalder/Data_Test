...
survivor_age_mean <- function(default=df){
        ...
        sboa <-paste ("The mean age of the survivors was :",colMeans(Subage,na.rm = TRUE),"so we can say that the passengers around the age of :", round(colMeans(Subage,na.rm = TRUE),digits = -1),"had higher changes of survival")
        ...
        graph_age <-ggplot(data = plot_age,aes(x=Age,fill=Survived))+geom_histogram(binwidth =3)
